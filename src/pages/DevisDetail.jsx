...
export default function DevisDetail() {
  ...
  // Enrichir le devis avec le PDF fabricant du produit (chargé à la volée)
  const getDevisWithProduitPdf = () => {
    const produits = useProductStore.getState().produits || []
    const prod = produits.find(p => p.id === devis.produitId)
    return { ...devis, produitPdfLocal: prod?.pdfLocal || null, produitPdfUrl: prod?.pdfUrl || null }
  }

  const handlePdfOpen=async(idx)=>{setPdfMsg('Génération...');try{await pdfService.ouvrirPDF(getDevisWithProduitPdf(),idx);setPdfMsg('')}catch(e){setPdfMsg('Erreur: '+e.message)}}
  const handlePdfDL=async(idx)=>{setPdfMsg('Téléchargement...');try{await pdfService.telechargerPDF(getDevisWithProduitPdf(),idx);setPdfMsg('')}catch(e){setPdfMsg('Erreur: '+e.message)}}
