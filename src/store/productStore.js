...
// Bloc standard 20×20×50 cm → surface couverte par bloc = 0.50 × 0.20 = 0.10 m²
// Épaisseur mur = 0.20 m, débord autour cuves = 0.40 m de chaque côté
export function calcBlocsABancher(produit) {
  const cuves = produit?.cuves || []
  if (cuves.length === 0) return null

  const DEBORD = 0.40      // 40 cm de chaque côté
  const EPAISSEUR = 0.20   // épaisseur bloc
  const BLOC_L = 0.50      // longueur d'un bloc (50 cm)
  const BLOC_H = 0.20      // hauteur d'un bloc (20 cm)
  const ENTRE_CUVES = 0.50 // 50 cm entre chaque cuve

  let perimetreInt, hauteurMur

  if (cuves.length === 1) {
    // 1 cuve : périmètre extérieur cuve + 40cm de chaque côté = périmètre intérieur mur
    const L = parseFloat(cuves[0].longueur) || 0
    const l = parseFloat(cuves[0].largeur) || 0
    const H = parseFloat(cuves[0].hauteur) || 0
    if (!L || !l || !H) return null
    const Lint = L + DEBORD * 2  // longueur intérieure
    const lint = l + DEBORD * 2  // largeur intérieure
    perimetreInt = 2 * Lint + 2 * lint
    hauteurMur = H + DEBORD
  } else {
    // Plusieurs cuves : garder la largeur d'une cuve, additionner les longueurs + 50cm entre chaque
    const largeurCuve = parseFloat(cuves[0].largeur) || 0
    let longueurTotale = 0
    let hauteurMax = 0
    for (let i = 0; i < cuves.length; i++) {
      longueurTotale += parseFloat(cuves[i].longueur) || 0
      const h = parseFloat(cuves[i].hauteur) || 0
      if (h > hauteurMax) hauteurMax = h
      if (i > 0) longueurTotale += ENTRE_CUVES  // 50cm entre chaque cuve
    }
    // Périmètre C = 2 × longueur totale + 2 × largeur cuve
    const perimC = 2 * longueurTotale + 2 * largeurCuve
    // Élargir de 40cm de chaque côté → ajoute 8 × 0.40 (4 coins × 2 côtés)
    // En fait : périmètre intérieur = 2 × (longueurTotale + 2×DEBORD) + 2 × (largeurCuve + 2×DEBORD)
    perimetreInt = 2 * (longueurTotale + DEBORD * 2) + 2 * (largeurCuve + DEBORD * 2)
    hauteurMur = hauteurMax + DEBORD
  }

  // Périmètre extérieur = périmètre intérieur + 8 × épaisseur (4 coins ajoutent 2 × épaisseur chacun)
  const perimetreExt = perimetreInt + 8 * EPAISSEUR
  // Surface du mur = périmètre extérieur moyen × hauteur
  const perimetreMoyen = (perimetreInt + perimetreExt) / 2
  const surfaceMur = perimetreMoyen * hauteurMur
  // Surface d'un bloc = 0.50 × 0.20 = 0.10 m²
  const surfaceBloc = BLOC_L * BLOC_H
  const nbBlocs = Math.ceil(surfaceMur / surfaceBloc)

  // Nb de rangs en hauteur et blocs par rang (pour info)
  const nbRangs = Math.ceil(hauteurMur / BLOC_H)
  const blocsParRang = Math.ceil(perimetreMoyen / BLOC_L)

  return {
    nbBlocs,
    perimetreInt: Math.round(perimetreInt * 100) / 100,
    perimetreExt: Math.round(perimetreExt * 100) / 100,
    hauteurMur: Math.round(hauteurMur * 100) / 100,
